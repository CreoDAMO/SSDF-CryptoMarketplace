...
describe("Marketplace Escrow + NFT Pair", function () {
  ...
  it("Deposits and releases standard (non-NFT) successfully", async function () {
    const tx = await escrow.connect(buyer).deposit(ORDER_ID, seller.address, AMOUNT, TIMEOUT, false, "", 0);
  ...
  it("Deposits and releases NFT atomically", async function () {
    await escrow.connect(buyer).deposit(ORDER_ID, seller.address, AMOUNT, TIMEOUT, true, TOKEN_URI, ROYALTY_BPS);
    await expect(escrow.connect(buyer).release(ORDER_ID))
      .to.emit(escrow, "Released")
      .withArgs(ORDER_ID)
  ...
  it("Handles disputes and time-locked admin refunds", async function () {
    await escrow.connect(buyer).deposit(ORDER_ID, seller.address, AMOUNT, TIMEOUT, false, "", 0);
